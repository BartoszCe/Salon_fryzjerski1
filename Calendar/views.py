...
def convert_data(activities_object):
    ...
    for i in activities_object:
        data.append({'id':i.id, 'title':i.__str__(), 'color':i.color, 'date':str(i.date.day),
            'start_time':str(int(str(i.start_time)[0:2]))})
    return data
...
def get_data(request, id):
    ...
    try:
        ...
        data = Activity.objects.filter(title=data.title, start_time=data.start_time).first()
    except:
        return HttpResponseNotFound()
    data = {'id':data.id, 'title':data.title, 'phone':data.phone, 'date':data.date.__str__(), 'start_time':data.start_time.__str__(),
            'employee':data.employee, 'color':data.color, 'private':data.private, 'comment':data.comment}
